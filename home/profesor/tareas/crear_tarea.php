...
$stmt = $pdo->prepare("
    ...
    FROM materias 
    WHERE id_materia = :id AND id_usuario_maestro = :m
");
$stmt->execute([":id" => $id_materia, ":m" => $id_maestro]);
$materia = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$materia) {
    die("No tienes permiso para crear tareas aquí.");
}
?>
<!DOCTYPE html>
<html lang="es">
...
<?php include "../../../includes/alertas_profesor.php"; ?>
    <h3 class="mb-3">Nueva tarea — <span class="text-primary"><?= htmlspecialchars($materia["materia_nombre"]); ?></span></h3>
